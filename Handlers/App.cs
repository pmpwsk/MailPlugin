                        ...
                        if (folder.Count != 0)
                        {
                            ...
                            foreach (var mId in GetLastReversed(folder, MessagePreloadCount, offset))
                                if (mailbox.Messages.TryGetValue(mId, out var m))
                                    e.Add(new ButtonElement(m.Subject, m.From.ContactString(mailbox) + "<br/>" + DateTimeString(AdjustDateTime(req, m.TimestampUtc)), $"{pluginHome}?mailbox={mailboxId}&folder={HttpUtility.UrlEncode(folderName)}&message={mId}{offsetQuery}", m.Unread ? "red" : null));
