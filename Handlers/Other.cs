        ...
        switch (req.Path)
        {
            ...
            { CreatePage(req, "Mail", out var page, out var e);
                ...
                {/////
                    ...
                    if (message.Unread)
                    {
                        await Mailboxes.TransactionIgnoreNullAsync(mailbox.Id, t =>
                        {
                            if (t.Value.Messages.TryGetValue(messageId, out var m))
                                m.Unread = false;
                        });
            ...
            { POST(req);
                ...
                if (folderName == "Trash")
                {
                    DeleteMessage(mailbox, t.FileActions, messageId);
                }
                else
                {
